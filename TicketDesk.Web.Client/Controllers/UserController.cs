...
namespace TicketDesk.Web.Client.Controllers
{
    ...
    public class UserController : Controller
    {
        ...
        [ValidateAntiForgeryToken]
        [Route("forget-password")]
        public async Task<ActionResult> ForgotPassword(UserForgetPasswordViewModel model, string returnUrl)
        {
            ViewBag.IsDemoMode = (ConfigurationManager.AppSettings["ticketdesk:DemoModeEnabled"] ?? "false").Equals("true", StringComparison.InvariantCultureIgnoreCase);
            ...
            if (user == null)
            {
                ...
                ModelState.AddModelError("UserEmail", Validation.InvalidEmail);
                return View("ForgetPassword");
            }
            String newPassword = "pass@123"; //"<PasswordAsTypedByUser>";
            var code = await UserManager.GeneratePasswordResetTokenAsync(user.Id);
            var callbackUrl = Url.Action("sign-in", "User", new { UserId = user.Id, code = code }, protocol: Request.Url.Scheme);
            string body = "<div> New password: " + newPassword + "</br>. Please reset your password by clicking here: <a href=\"" + callbackUrl + "\">link</a>" + " </div>";
            var isOk = CommonHelper.SendContentMail(user.Email, body, null, "Reset Password", null);
            ...
            if (!string.IsNullOrEmpty(domain))
            {
                if (user == null)
                {
                    user = await UserManager.FindByNameAsync(model.UserEmail + "@" + domain);
                }
