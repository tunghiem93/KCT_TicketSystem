...
namespace TicketDesk.Web.Client.Controllers
{
    ...
    public class SearchController : Controller
    {
        ...
        public async Task<ActionResult> Index(string term)
        {
            ...
            var projectIdHome = HttpContext.Request.Params.Get("ProjectId");
            ...
            if (projectIdHome != "-1" && term != null)
            {
                projectId = Convert.ToInt32(projectIdHome);
                var projectName = Context.Projects.FirstOrDefault(f=>f.ProjectId.Equals(projectId)).ProjectName;
                term = projectName;
                var model = await TdSearchContext.Current.SearchAsync(Context.Tickets.Include(t => t.Project), term, projectId);
