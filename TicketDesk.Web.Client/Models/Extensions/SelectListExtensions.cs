...
namespace TicketDesk.Web.Client
{
    ...
    public static class SelectListExtensions
    {
       ...
       (
        ...
        public static SelectList ToSelectListProject<TSource, TValue>
        (
            this IEnumerable<TSource> items,
            Func<TSource, TValue> valueField,
            Func<TSource, string> textField,
        ...
        {
            var list = items.Select(i => new SelectListQueryItemProject<TValue> { ProjectId = valueField(i), ProjectName = textField(i) }).ToList();
            list.Insert(0, new SelectListQueryItemProject<TValue> { ProjectId = (TValue)Convert.ChangeType(-1, typeof(TValue)), ProjectName = DefaultItemText });
            //AddDefaultItemProject(list, includeDefaultItem);
            return new SelectList(list, "ProjectId", "ProjectName", selectedValue);
        }
       ...
       (
        ...
        private static SelectListQueryItemProject<T> GetDefaultSelectListItemProject<T>()
        {
            return new SelectListQueryItemProject<T> { ProjectName = DefaultItemText, ProjectId = default(T) };
        }
        ...
        internal class SelectListQueryItemProject<T>
        {
        ...
        private static void AddDefaultItemProject<T>(IList<SelectListQueryItemProject<T>> list, bool includeDefaultItem)
        {
            if (includeDefaultItem)
            {
                list.Insert(0, GetDefaultSelectListItemProject<T>());
            }
        }
