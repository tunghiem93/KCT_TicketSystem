...
namespace TicketDesk.Web.Client.Models
{
    public class FilterBarViewModel
    {
        ...
        public SelectList TicketProjectSelectList
        {
            get
            {
                string selectedProject = "0";
                var fColumn = CurrentListSetting.FilterColumns.SingleOrDefault(fc => fc.ColumnName == "ProjectId");
                if (fColumn != null && fColumn.ColumnValue != null)
                {
                    selectedProject = fColumn.ColumnValue.ToString() == null ? "0" : fColumn.ColumnValue.ToString();
                }
                var context = DependencyResolver.Current.GetService<TdDomainContext>();
                var lProjects = context.Projects.OrderBy(p => p.ProjectName)
                    .ToSelectListProject(p => p.ProjectId.ToString(), p => p.ProjectName, true );
                return new SelectList(lProjects.Items, "ProjectId", "ProjectName", selectedProject);
            }
        }

        public Dictionary<string, object> ProjectFilterHtmlAttributes
        {
            get
            {
                var isProjectToFilterDisabled = CurrentListSetting.DisabledFilterColumnNames.Contains("ProjectId");
                var cVal = String.Format("postback{0} form-control", (isProjectToFilterDisabled) ? " disabled" : string.Empty);
                var projectHtmlAttributes = new Dictionary<string, object> { { "Class", cVal } };

                return projectHtmlAttributes;
            }
        }
            ...
            get
            {
                ...
                return new SelectList(
                    ...
                    {   
                        ...
                        new { Text=Strings.TicketStatus_MoreInfo, Value="Inprogress"},
        ...
        public SelectList DaterangeToSelectList
        {
            get
            {
                string selectedUserName = "Daterange";
                var fColumn = CurrentListSetting.FilterColumns.SingleOrDefault(fc => fc.ColumnName == "AssignedTo");
                if (fColumn != null)
                {
                    //when filter for column exists, but the value is null it means the selection was unassigned
        ...
        public Dictionary<string, object> DaterangeFilterHtmlAttributes
        {
            get
            {
                var isAssignedToFilterDisabled = CurrentListSetting.DisabledFilterColumnNames.Contains("AssignedTo");
